// ====================================================================
// WORKER V6 - VERCEL EDITION (Edge Function)
...
// ==================== IN-MEMORY CACHE (untuk Vercel) ====================
// Karena Vercel Edge tidak memiliki caches.default, kita gunakan Map global.
// Perhatikan: ini tidak persisten antar instance, hanya untuk demo.
...
export default async function handler(request, context) {
  ...
  // Ambil / inisialisasi stats worker dari cache memory
  ...
  for (let i = 0; i < topWorkers.length; i += batchSize) {
    ...
    try {
      ...
      // ========== 5. PASSIVE HEALTH CHECK ==========
      // Jalankan di latar belakang jika context.waitUntil tersedia
      const healthCheckPromise = performPassiveHealthCheck(workerStats);
      if (context && typeof context.waitUntil === 'function') {
        context.waitUntil(healthCheckPromise);
      } else {
        // Fallback: jalankan tanpa menunggu (fire-and-forget)
        healthCheckPromise.catch(() => {});
      }

      // Simpan stats secara async
      const savePromise = saveWorkerStats();
      if (context && typeof context.waitUntil === 'function') {
  ...
  // Semua gagal
  saveWorkerStats().catch(() => {});
  return new Response('Semua worker gagal. Coba lagi nanti.', { status: 503 });
}

// ==================== FUNGSI PEMBANTU (dengan in-memory cache) ====================

/**
 * Ambil stats dari Map global
 */
...
/**
 * Simpan stats ke Map global
 */
async function saveWorkerStats() {
  try {
    const stats = getWorkerStats();
    if (stats) {
      statsCache.set(CACHE_KEY, JSON.stringify(stats));
    }
  } catch (_) { /* ignore */ }
}

/**
 * Set stats (digunakan untuk inisialisasi)
 */
 ...
 */
async function performPassiveHealthCheck(workerStats) {
  const now = Date.now();
